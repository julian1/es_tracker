...
## psql -d test -c 'select id, data::json->$$timestamp$$  from queue'
...
while true do
    ...
    open("https://www.bitstamp.net/api/order_book/", "rb") do |read_file|
      res = conn.exec_params( 'insert into queue( t, msg, data) values( now()::timestamptz, $$order$$, $1::json )', 
        [read_file.read] )
    end

    open("https://www.bitstamp.net/api/ticker/", "rb") do |read_file|
      res = conn.exec_params( 'insert into queue( t, msg, data) values( now()::timestamptz, $$ticker$$, $1::json )', 
        [read_file.read] )
    end
  rescue
    begin
      # On error just record the error
      res = conn.exec_params( 'insert into queue( t, msg, data) values( now()::timestamptz, $$error$$, $1::json )', 
          [$!.to_json] )
    rescue
